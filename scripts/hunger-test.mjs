...
// ----------------------------------------------------------------- the eater
{
  ...
  // The cancel has to take effect *at the cancel*, not at the next update.
  // The game loop lets go of the button, calls cancel(), and then asks
  // `eating` whether the hands are free to dig and place again — and it has
  // no reason to call update() on a meal it just abandoned. A cancel that
  // only set a flag would leave the eater busy for the rest of the session
  // and quietly disable mining and building with it.
  const dropped = new Eater();
  dropped.start(APPLE);
  dropped.update(EAT_SECONDS / 4);
  dropped.cancel();
  assert('cancel frees the hands immediately, with no update in between',
    !dropped.eating && dropped.fraction === 0);
  dropped.cancel();
  assert('...and cancelling nothing stays nothing', !dropped.eating);
  // The interrupt is still reported once, to whoever asks next.
  const late = dropped.update(0);
  assert('...and the interrupt is still there to be read once',
    late && late.done === false && late.id === APPLE);
  assert('...only once', dropped.update(0) === null);

  // Starting a fresh meal clears an unread interrupt rather than serving it
  // up as the result of the new mouthful.
  const reused = new Eater();
  reused.start(STEAK);
  reused.cancel();
  reused.start(APPLE);
  assert('a new meal is not haunted by the cancelled one',
    reused.eating && reused.id === APPLE && reused.update(0) === null);
