...
627 headless checks across nine suites: Minecraft movement speeds, jump height, wall collision, auto jump, the swimming pose, ice friction, fall damage and drowning; infinite-world chunking (negative coordinates, generation, eviction, edits surviving a regenerate), caves and the deepslate transition; the mesher (face culling, chunk seams, atlas UVs); the inventory model; block targeting; the save/settings/camera systems; pointer lock plus the native iPad bridge; water, in three halves — how far it spreads, whether it finds the hole, what a drop does to it, what it refuses to touch; the geometry it turns into; and where it freezes, which is its own set of rules to get wrong; and the weather, which is a clock and a climate and has to be right about both.
