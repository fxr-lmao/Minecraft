...
- **...but digging takes time.** It used to be a tap — point at a mountain and it came apart as fast as you could click, which is nearer to deleting than to mining, and it meant nothing in the world cost anything. Minecraft's rule is one line, `hardness × (right tool ? 1.5 : 5) ÷ speed`, and everything else is a table: stone by hand is seven and a half seconds and with a pickaxe it is three eighths of one, dirt comes away in a moment and quicker with a shovel, and deepslate is twice the stone. The penalty for the wrong tool is not that it is slower — it is that it is *three and a third times* slower, exactly as if you were using your hands, and that ratio is the whole reason a pickaxe is worth carrying. A crack grows over the block while you hold the button, in four stages that accumulate rather than take turns, and it is drawn as a pale line with a dark one a pixel up and left of it: black alone was the obvious way to do it and it was wrong, because mining happens underground, where the rock is already nearly black and a black line on it is nothing at all. Look away, or let go, and the progress is gone with it — you were digging *that* block. And ore broken with anything other than a pickaxe breaks for nothing at all, so the vein you tear into while holding a stack of dirt is simply gone; the hotbar starts with a pickaxe and a shovel in it so that the rule is a thing to remember rather than a wall
...
684 headless checks across ten suites: Minecraft movement speeds, jump height, wall collision, auto jump, the swimming pose, ice friction, fall damage and drowning; infinite-world chunking (negative coordinates, generation, eviction, edits surviving a regenerate), caves and the deepslate transition; the mesher (face culling, chunk seams, atlas UVs); the inventory model; block targeting; the save/settings/camera systems; pointer lock plus the native iPad bridge; water, in three halves — how far it spreads, whether it finds the hole, what a drop does to it, what it refuses to touch; the geometry it turns into; and where it freezes, which is its own set of rules to get wrong; the weather, which is a clock and a climate and has to be right about both; and mining — how long every block takes with every tool, what it leaves behind, and where in the rock the ore is.
...
Cave entrances, lava, crafting and a furnace, swords and something to swing them at, more block types, sounds.
...
- **Drowning and falling are the only two ways to die.** There is nothing else in the world that can hurt you — no mobs, no lava, no starving — and no armour to soften either of them.
...
- **Health comes back on its own**, which Minecraft would do with food. Adding hunger means a bar, a saturation model and something to eat, and none of those are water.
