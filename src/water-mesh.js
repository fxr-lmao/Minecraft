 ...
 * fractions of a block), normals are the usual -1/0/1, and UVs are 16-bit
 * normalised, which is a thousandth of a texel and far finer than anything
 * here needs.
 */
class QuadBuffer {
   ...
   * outside. `v` holds x,y,z per corner and `t` holds u,v per corner, both
   * flat; they are scratch arrays owned by the mesher, never allocated here.
   */
  quad(v, t, nx, ny, nz) {
...
export class FluidMesher {
   ...
   *     level with itself rather than being pulled down by the rock.
   */
  corner(cx, layer, cz) {
    const slot = cz * (CHUNK_SIZE + 1) + cx;
    if (this.cornerLayer[slot] === layer) return this.cornerHeight[slot];
    const height = fluidCornerHeight(this._get, cx, layer, cz);
    this.cornerLayer[slot] = layer;
    this.cornerHeight[slot] = height;
    return height;
  }
  ...
  cell(lx, layer, lz, id) {
    ...
    // The four corner heights, in Minecraft's order: north-west, south-west,
    // south-east, north-east, with north being -z and west -x.
    const h = this._h;
    h[NW] = this.corner(lx, layer, lz);
    h[SW] = this.corner(lx, layer, lz + 1);
    h[SE] = this.corner(lx + 1, layer, lz + 1);
    h[NE] = this.corner(lx + 1, layer, lz);
  ...
  _top(lx, layer, lz, id, h) {
    ...
    if (speed === 0) {
      ...
      t[4] = 1; t[5] = 1;
      t[6] = 1; t[7] = 0;
      this.buffers[STILL].quad(v, t, 0, 1, 0);
      return 1;
    }
    ...
    this.buffers[FLOWING].quad(v, t, 0, 1, 0);
    return 1;
  }
  ...
  _sides(lx, layer, lz, h, mask) {
    ...
    for (let d = 0; d < 4; d++) {
      ...
      t[4] = 1; t[5] = 0.5;
      t[6] = 0; t[7] = 0.5;
      this.buffers[FLOWING].quad(v, t, e.nx, 0, e.nz);
      quads++;
    }
    return quads;
  }
...
export function fluidCornerHeight(get, x, y, z) {
  ...
  for (let j = 0; j < 4; j++) {
    const cx = x - (j & 1);
    const cz = z - ((j >> 1) & 1);
    if (isWater(get(cx, y + 1, cz))) return 1; // a column, not a surface
    ...
    if (isWater(id)) {
      ...
      weight += 1; // air: pulls the corner down toward nothing
    }
  }
