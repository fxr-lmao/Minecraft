...
export class Eater {
  constructor() {
    ...
    this.progress = 0;
    /**
     * An interrupted meal, waiting to be reported by the next `update`.
     * The meal itself ends the instant it is cancelled — `eating` goes false
     * and the caller's hands are free again — but the *report* has to
     * survive until someone asks for it, because the loop that cancels a
     * meal (the button came up) is not the loop that reads the result.
     */
   ...
   *
   * The meal ends *here*, not on the next `update`: the caller that cancels
   * is the one that let go of the button, and it goes on to ask `eating`
   * whether the hands are free — so a cancel that only set a flag would
   * leave the eater looking busy forever and the player unable to dig or
   * place. The interrupt report is stashed for the next `update` (and
   * returned here too) so nothing is lost by ending it early.
   *
   * @returns {null | {done: false, id: number, info: object, poison: false}}
   */
  cancel() {
    if (!this.id) return null;
    this._interrupted = { done: false, id: this.id, info: FOOD[this.id], poison: false };
    this.id = 0;
    this.progress = 0;
  ...
  update(dt, rand = Math.random) {
    if (this._interrupted) {
      const report = this._interrupted;
      this._interrupted = null;
      return report;
    }
    if (!this.id) return null;
