...
export class Player {
   ...
   */
  hurt(amount, cause, { armoured = true } = {}) {
    ...
    //
    // `armoured: false` is the escape hatch for the two things a suit of
    // iron is no help against: drowning and starving. It is Minecraft's rule
    // (armour covers blows, not the absence of air or food), and here it is
    // also load-bearing, because those two arrive as a *rate* rather than a
    // blow — drowning calls this once per physics step — and a piece of
    // armour spends at least one use on every hit it softens. Run 2 hp/s of
    // drowning through the armour at 120 steps a second and a full suit of
    // iron is scrap in under a second and a half of swimming.
    if (armoured && this.armour && this.armour.some(Boolean)) {
  ...
  _breathe(dt) {
    ...
    if (this.submerged) {
      this.air -= dt;
      if (this.air < 0) this.hurt(DROWN_DAMAGE * dt, 'drowned', { armoured: false });
    ...
    if (s.starve > 0) this.hurt(s.starve, 'starved to death', { armoured: false });
  }
