...
import { totalArmourPoints, isArmour, ARMOUR_SLOTS } from './armour.js';
...
import { STARTER_BLOCKS, getAtlasTexture, getCrackStages, crackStage, blockTint, blockName } from './textures.js';
...
import { spendDurability, ensureDurability, isTool } from './durability.js';
...
if (restored?.player) {
  ...
  if (Array.isArray(restored.player.armour)) {
    // Written back into the array the inventory already owns, one slot at a
    // time, rather than replacing it: the player holds a reference to that
    // same array (see below) and swapping it out here would leave the two
    // looking at different armour — the HUD drawing a chestplate that stops
    // nothing, and a piece that broke on your back still hanging in the
    // inventory screen. A short or long save fills what it can and no more.
    for (let i = 0; i < ARMOUR_SLOTS; i++) {
      const raw = restored.player.armour[i];
      inventory.armour[i] = Array.isArray(raw) && raw[0]
        ? ensureDurability({
          id: raw[0], count: 1,
          durability: Number.isFinite(raw[1]) && raw[1] > 0 ? raw[1] : undefined,
        })
        : null;
    }
  }
...
// The armour the player wears lives in the inventory's four slots, and the
// player reads that very array whenever it is hurt — so it is handed over
// after the save has been restored into it, never before.
...
function useHeldItem(stack) {
  // Right-clicking a piece of armour puts it on, which is how Minecraft
  // equips one and the only way to do it without opening the inventory.
  if (isArmour(stack.id)) {
    if (inventory.equipArmour(inventory.selected)) {
      hud.showStatus(`${blockName(stack.id)} equipped`);
      invUI.render();
      saveNeeded = true;
    } else {
      hud.showStatus('That slot is already covered');
    }
    return;
  }
  // A bow with nothing to shoot looks exactly like a bow that does not work,
  // so it says which one it is.
  if (stack.id === BOW) {
    hud.showStatus('You have no arrows');
    return;
  }
  const dir = player.lookDirection();
...
function fireBow(power) {
  ...
  swingHand();
  // Creative has infinite everything else; it has infinite arrows too.
  if (!gameMode.isCreative) inventory.consume(ARROW, 1);
  const stack = inventory.selectedStack();
...
function frame(now) {
  ...
  if (playing && !screenOpen() && input.pressed.place) {
    if (handStack && isFood(handStack.id) && player.hunger.canEat) {
      // Scrolling to a different food mid-mouthful starts that food's meal
      // instead of finishing the last one on its behalf — otherwise a bite
      // of rotten flesh could be paid for out of the steak you switched to.
      if (eater.eating && eater.id !== handStack.id) eater.cancel();
