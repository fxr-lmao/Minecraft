...
export class Mobs {
  ...
  _move(mob, dt, world, intent) {
    ...
    // Pushed into something while meaning to walk. Every ground mob climbs
    // one block by hopping off the floor — it is why hills do not stop a
    // zombie — and none of them climbs two.
    //
    // A spider does not hop, and this is the one place that difference has
    // to be written carefully. A hop is an *impulse*: JUMP_VELOCITY buys
    // 8.95²/2g ≈ 1.25 blocks, exactly the one step it is meant to buy. A
    // climb is a *rate*, and giving it to gravity as an impulse the same way
    // buys 3.2²/2g ≈ 0.16 blocks — less than a doorstep, so a "climber"
    // written that way is worse at walls than the zombie beside it. So the
    // climb is re-applied every tick the spider is still pressing into the
    // wall, with no floor required, which is what climbing actually is: it
    // goes up for as long as there is something in front of it, and a wall
    // is the one thing that will not save you from a spider.
    const pressing = intent && (intent.x !== 0 || intent.z !== 0);
    if ((moved.blockedX || moved.blockedZ) && pressing) {
      if (mob.facts.climber) {
        mob.vy = SPIDER_CLIMB_SPEED;
        mob.onGround = false;
        // Nothing that went up a wall under its own power has been falling.
        mob.fallDistance = 0;
